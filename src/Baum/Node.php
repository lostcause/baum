...
abstract class Node extends Model
{
	...
	{
		return $this->hasMany(get_class($this), $this->getParentColumnName())->orderBy($this->getOrderColumnName());
	}
	...
	public static function all($columns = array('*'))
	{
		...
		return $instance->newQuery()->orderBy($instance->getQualifiedOrderColumnName())->get();
	}
	...
	public static function roots()
	{
		...
		return $instance->newQuery()->whereNull($instance->getParentColumnName())->orderBy($instance->getQualifiedOrderColumnName());
	}
	...
	public static function allLeaves()
	{
		...
		return $instance->newQuery()->whereRaw($rgtCol . ' - ' . $lftCol . ' = 1')->orderBy($instance->getQualifiedOrderColumnName());
	}
	...
	public function trunks()
	{
		...
		return $this->descendants()->whereNotNull($this->getQualifiedParentColumnName())->whereRaw($rgtCol . ' - ' . $lftCol . ' != 1');
	}
	...
	public function restoreDescendants()
	{
		...
		$this->getConnection()->transaction(function () use ($self)
		{
			$self->newNestedSetQuery()->withTrashed()->where($self->getLeftColumnName(), '>', $self->getLeft())->where($self->getRightColumnName(), '<', $self->getRight())
				 ->update(array($self->getDeletedAtColumn() => null,
